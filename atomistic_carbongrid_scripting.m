...
hcen = [size(boxsize,1)/2+randi(50),radius+20+randi(10)-pix*1]; %offsets for the hole center
...
h = sqrt( (pts(1,:)-hcen(1)).^2 + (pts(2,:)-hcen(2)).^2 ) +rand(ccount,1)*0;
carbons2 = round(carbons(h>radius,:)+0.5); %filter out points inside the grid hole
